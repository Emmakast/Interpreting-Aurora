...
def plot_scatter(scores_dict, x_idx, y_idx, var_ratio, title, filename):
    ...
    for label, tensor in scores_dict.items():
        ...
        plt.scatter(center[x_idx], center[y_idx], c='black', marker='X', s=200, linewidth=2, edgecolors='white', zorder=10)
        plt.text(center[x_idx], center[y_idx], f"  {label}", fontsize=14, fontweight='bold', 
...
def plot_loadings(components, title, filename):
    ...
    plt.ylabel("Absolute Loading", fontsize=14)
    plt.xticks(fontsize=12)
    plt.yticks(fontsize=12)
...
def plot_contrastive_pair(score_dict, label_a, label_b, color_a, color_b, filename):
    ...
    plt.xlabel("Score along Separation Axis", fontsize=14)
...
def main():
    ...
    if args.mode == "seasonal":
        ...
        # Handle variance ratio - compute from explained_variance if not present
        if "explained_variance_ratio" in data:
            var_ratio = data["explained_variance_ratio"].numpy()
        else:
            var = data["explained_variance"].numpy()
            var_ratio = var / var.sum()
        components = data["components"].numpy()
    else:
        # Storm mode - check for various key formats
        if "scores_storm" in data:
            # storm_calm_pooled_results.pt format (CORRECT ONE)
            scores_dict["storm"] = data["scores_storm"]
            scores_dict["calm"] = data["scores_calm"]
            var_ratio = data["explained_variance_ratio"].numpy()
            components = data["components"].numpy()
        elif "combined_scores_storm" in data:
            # joint_vertical_pca_results_old.pt format
            scores_dict["storm"] = data["combined_scores_storm"]
            scores_dict["calm"] = data["combined_scores_calm"]
            var = data["combined_explained_variance"].numpy()
            var_ratio = var / var.sum()
            if "combined_components" in data:
                components = data["combined_components"].numpy()
            else:
                n_features = scores_dict["storm"].shape[1]
                components = np.eye(min(3, n_features), n_features)
        elif "storm_pca" in data:
            # aurora_storm_calm_pca.pt format
            scores_dict["storm"] = data["storm_pca"]
            scores_dict["calm"] = data["calm_pca"]
            if "storm_var" in data:
                var = data["storm_var"].numpy()
                var_ratio = var / var.sum()
            else:
            ...
            print("Unknown storm PCA format")
            return
    ...
    if args.mode == "seasonal":
        ...
        plt.xlabel("Contrastive Score", fontsize=14)
        plt.ylabel("Density", fontsize=14)
        plt.xticks(fontsize=12)
        plt.yticks(fontsize=12)
        plt.legend(fontsize=14)
